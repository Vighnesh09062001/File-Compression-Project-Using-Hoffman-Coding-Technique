...
public class HuffCompression {
    ...
    public static void compress(String src, String dst) {
    	//input source and Destination as a String 
        ...
        try {
            ...
            inStream.read(b);
            //if I read the character then the size of the compress massage is going to be increase
            //so i don't want to increase size.
    ...
    //it is accepting the original Bytes in their to create new min priority queue.
    private static MinPriorityQueue<ByteNode> getByteNodes(byte[] bytes) { 
        ...
        //calculate each character frequency.
        for (byte b : bytes) {
    ...
    //here passing root reverse left 0 and root reverse to right 1
    ...
    private static void getHuffCodes(ByteNode node, String code, StringBuilder sb1) {
        ...
        if (node != null) {
            if (node.data == null) {
            	...
            	//new hashmap in which character and its new sequence.
                huffmap.put(node.data, sb2.toString());
        }
    }

    //return Byte Array which is going to be intialized 
    private static byte[] zipBytesWithCodes(byte[] bytes, Map<Byte, String> huffCodes) {
        StringBuilder strBuilder = new StringBuilder();
        for (byte b : bytes)
            strBuilder.append(huffCodes.get(b));
        // cutting this 8 group
        int length=(strBuilder.length()+7)/8;
        //storing 8 bits sequence of char 
        byte[] huffCodeBytes = new byte[length];
        int idx = 0;
        //appending  on string then passing this to string into integer byte storing into huffcodes 
        //+bytes  & return huffcode
    ...
    public static void main(String[] args) {
    	compress("C:\\Users\\eXpert\\Desktop\\WEB TECHNOLOGY.txt", "C:\\Users\\eXpert\\Desktop\\file.txt");
    }
}